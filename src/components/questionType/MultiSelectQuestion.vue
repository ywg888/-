<template>
	...
	>
		<div class="problem-box" @click.stop="beActive">
			...
			<el-checkbox-group v-model="multiSelect" class="select-group">
				<li
					v-for="(item, index) in problemData.setting?.options"
					:key="index"
					class="select"
				>
					<el-checkbox disabled :label="item.title" size="large" class="label"
						><input
							v-model="item.title"
			...
			<div class="footer-opts">
				...
				<el-dialog v-model="dialogFormVisible" title="预设答案">
					<el-select v-model="multiSelect" multiple placeholder="请选择">
						<el-option
							v-for="(item, index) in problemData.setting?.options"
							:key="index"
							:label="item.title"
